...
async def ensure_house_runtime(showing_id: int) -> None:
    ...
    async with _sim_lock:
        ...
        if os.environ.get("SIMULATE_PATRONS", "1") != "0":
            tasks.extend(
...
async def _publish_locked(showing_id: int) -> None:
    ...
    order the EVALs actually resolved in.
    """
...
async def _simulate_patron(showing_id: int, index: int) -> None:
    ...
    while True:
        try:
            ...
            fresh = await read_seat_map(app.state.redis, showing_id, ids)
...
async def showing_state(
    ...
    ids = [row.seat_id for row in rows]
    live = await read_seat_map(app.state.redis, showing_id, ids)
    stats = await read_stats(app.state.redis, showing_id)
    stats["double_bookings"] = await asyncio.to_thread(
...
async def race(showing_id: int, seat_id: str, body: RaceRequest) -> dict:
    ...
    live = await read_seat_map(app.state.redis, showing_id, [seat_id])
